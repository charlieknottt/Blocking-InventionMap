          ...
          inventions: [
            {
              ...
              full: {
                ...
                superior: "Collecting many real photos per SKU is operationally expensive and hard to refresh. Pure 2D augmentation cannot fully model occluded sides, geometry, or pose-dependent appearance. Generic embeddings may not preserve the fine-grained SKU distinctions required for lookalike products. Training a network to regress a 3D shape directly from one image trades a cheap geometric prior for an expensive learned one, and such regressors exist only for faces, so a competitor would have to 3D-scan and train per geometry class — a very costly and time-consuming approach. As a result, Ultron's invention produces faster onboarding and stronger performance on unusual angles and visually similar SKUs, all of which allows Ultron to work at grocery scale."
              }
            },
            {
              id: "single-shot-3d",
              ...
              full: {
                accomplishes: "This extends the single-shot enrollment concept of Derived 3D Object Model from a Single 2D Image directly into grocery/product objects. It generates a 3D model from a single 2D image by classifying the object shape, segmenting the product outline with texture information, dimensionally sampling that outline, generating a shape-matched 3D mesh, rendering the texture onto the mesh, and exporting different viewpoints.",
                ...
                superior: "Conventional augmentation cannot reproduce true 3D geometry or occluded faces. NeRF-like multi-view generation needs multiple input views, which defeats single-image enrollment. Manual 3D modeling is too slow for a 40k+ SKU catalog, and store-capture accumulation leaves cold-start gaps and depends on live customer traffic. As a result, Ultron's invention produces faster and more scalable enrollment, keeps catalogs more current, and makes recognition stronger under real checkout presentation conditions, especially when package churn and long-tail SKUs are introduced."
              }
            },
            {
              id: "pose-tolerant",
      ...
      techAreas: [
        {
          ...
          inventions: [
            ...
            },
            {
              ...
              full: {
                accomplishes: "At a high level, these inventions improve detection of products that are small, partially hidden, or surrounded by clutter — the hard cases in a real checkout or shelf scene. Their distinctive element is contextual scoring: instead of judging a detection from the object's bounding box alone, they also pull features from surrounding context regions and use both to decide whether a product is really there and what it is. Technically, multi-layer features are used to propose a region; that region is projected back to the feature maps to obtain the object ROI and a corresponding context region, and the two are scored together to produce the detection confidence. This technology was key in our US Army evaluation, where it allowed us to detect very small faces, and was one of the contributing factors that led to UltronAI being awarded the “Outstanding Contributor in AI” from the Secretary of the Army in 2020.",
                ...
                superior: "A competitor could score detections from the object box alone, or obtain context through a different mechanism such as global attention. Box-only scoring is the genuinely weaker path: on small and occluded items it produces more misses and more low-confidence detections, exactly where local pixels run out. Attention-based context is a real alternative, but it trades the invention's explicit, localized context-region method — purpose-built for this case — for a heavier, less direct one. As a result, Ultron's invention reliably holds onto small, partially visible, and cluttered products that an ROI-only detector would drop, and because every detection failure caps everything downstream, those recovered small-item detections directly protect accuracy and shrink reduction at checkout scale."
              }
            },
            {
              ...
              full: {
                ...
                superior: "A competitor could fully annotate everything, or use one of several alternative training schemes that tolerate unlabeled objects. But those alternatives buy robustness by adding cost — extra training machinery, or coupling to a heavier detector that carries into inference. As a result, Ultron's invention is purely a loss-function change on a one-stage detector: the annotation-robustness lives entirely in training and costs nothing at runtime, so the model that ships is still the lean single-stage detector you started with. For edge deployment that is the whole point — a competitor matching this accuracy on messy retail data usually pays in training complexity, inference cost, or both, exactly where on-device budgets have the least room to give."
              }
            }
          ]
        },
          ...
          inventions: [
            {
              ...
              full: {
                ...
                superior: "A competitor could use rotated bounding boxes, axis-aligned boxes plus segmentation masks, or simply enlarge the crop and let the classifier absorb the noise. Rotated boxes are the closest alternative, but a product viewed at an angle is distorted by perspective, so its visible face is a general quadrilateral rather than a rotated rectangle — a rotated box still lets in background or clips the package on tilted, off-axis views. Segmentation is heavier to train and run, and enlarged crops pull in background and neighboring products. As a result, Ultron's invention gives the recognizer cleaner product evidence, reducing false positives on lookalike products and false negatives on partially visible or tilted products."
              }
            },
            {
              id: "concave-polygons",
              ...
              full: {
                accomplishes: "This invention extends localization beyond rectangles and even quadrilaterals to complex concave polygons. The case it addresses is the one a four-corner box still cannot fit: densely packed shelves where products overlap, and irregular or deformable items like bags and produce. When a neighboring product cuts into the box, or the visible region is simply not convex, a rectangle or quadrilateral pulls in background or adjacent-product pixels. A concave polygon can follow the target product's actual visible boundary and exclude the rest. Quadrilateral boxes (the sibling invention) already handle products viewed at an angle; this invention goes further, to visible regions that are not convex at all. Cleaner isolation gives the downstream classifier and feature matcher a less contaminated signal.",
                works: "In a real shelf or checkout scene, the visible evidence for a product is often not a clean rectangle, and depending on camera angle and product placement, not a rotated rectangle either. A box that does not fit carries background and neighboring-product pixels into the crop. Fitting a concave polygon to the product's visible region isolates it more precisely before feature extraction, so the matcher receives a cleaner, normalized crop. This matters most for partially occluded items and flexible packaging, where the visible shape is genuinely irregular.",
                superior: "A competitor could use rotated rectangles or instance segmentation. Rotated rectangles are convex, so on overlapping or irregular products they still include a neighbor's pixels or background. Segmentation can follow an arbitrary shape, but it is heavier to run, label-hungry to train, and returns a raw pixel mask rather than a structured boundary that can be used to normalize the crop, and it still depends on detection as a first step. As a result, Ultron's invention isolates true product evidence more precisely on exactly the dense, occluded cases common in retail, improving match accuracy while avoiding the added compute, memory, and latency that segmentation-based workarounds impose on edge hardware."
              }
            },
            {
              id: "homography",
              ...
              full: {
                accomplishes: "At a high level, this invention helps the system “straighten out” products that appear tilted or distorted in checkout or shelf images, making them easier to recognize. Picture walking down an aisle and looking at the shelves; the products are skewed as they recede from the camera. Scene rectification rectifies the shelf-capture image into a near-orthographic view, so products can be detected by simple rectangular object detectors. This is especially important when products are presented at angles or viewed from the side. Technically, it protects the use of homography estimation to rectify a scene or product view, transforming a tilted planar surface into a normalized, front-facing representation, so downstream detection and recognition operate on a more consistent and comparable view. The approach applies to camera scenes, product shelves, checkout counters, and similar settings.",
        ...
        },
        {
          ...
          inventions: [
            ...
            },
            {
              ...
              full: {
                accomplishes: "At a high level, this invention ensures the system can still identify products even when the initial image is a poorly angled view for a direct match. Instead of relying on a single attempt or image enrollment, the system can generate alternate versions of the enrolled product view (e.g., 3D rotated, 3D view adjusted, or otherwise transformed as a combination of 2D and 3D through any means) and use these to match the test image against the product library. Alternatively, this can be done dynamically on the fly or pre-computed; it can also take the test input image and generate alternate views to match to the enrollment. Technically, it expands the multi-hypothesis matching approach by transforming the observed product image or enrollment image, extracting features from those transformed views, and matching them directly against product representations — without requiring a successful first-pass match. It also covers aggregating the results of multiple hypothesis transformations into a final match confidence.",
                works: "In real checkout conditions, many frames are imperfect — products are tilted in three dimensions, often to a view not resembling the CPG image, or poorly lit, or partially occluded and only briefly visible. A system that relies on a single clean view will fail in these cases. Being able to take a single enrollment image and generate multiple 3D views at different angles is key to matching a test image fast at any angle; alternatively, the system can take the test image and dynamically hypothesize how it would look under various 3D and other transformations (occlusions, lighting) to match the pristine image. By generating alternate views for the match, the system finds a strong matching signal from otherwise unusable frames, and the aggregation across multiple transformation hypotheses can be fused over time. Strategically, this significantly increases the percentage of checkout interactions that result in a confident identification, improving throughput and reducing missed detections or shrink.",
                superior: "A competitor could rely on a single-pass recognition model that only matches the enrollment-image pose. It would then require the user to rotate the test object for a better match, or manually enroll input images at different angles and segment them into the database — which is not practical or scalable. As a result, Ultron's invention does both automatically: single-pass systems fail more often on difficult frames, and limiting transformations to post-match scenarios misses the exact cases where they are most needed. Routing those failures to staff creates friction, slows checkout, and increases labor cost, undermining the economic value and usability of the system."
              }
            },
            {
              id: "multiview",
          ...
          inventions: [
            {
              ...
              full: {
                ...
                superior: "A competitor could compare a vision prediction to the scanned code and flag mismatches, but that one-directional check is long-established; the verification here works in both directions and is scoped to the transaction, which is much harder to match on reliability than on logic. Alternatively, a competitor could rely primarily on barcode and weight-based systems with vision as a secondary check, but that decouples recognition from decision-making and leans on coarse rules that cannot handle partial visibility, lookalike substitutions, or multi-item interactions. As a result, Ultron's invention integrates visual evidence, transaction context, and confidence handling into a unified runtime decision system, producing better shrink detection while avoiding the excessive false alarms or missed events that hurt both customer experience and the business case."
              }
            },
            {
              id: "reverse-lookup",
          ...
          inventions: [
            {
              ...
              full: {
                ...
                superior: "Cloud inference introduces latency and connectivity risk, heavy edge hardware hurts deployment economics, and generic small models may lose accuracy on hard retail cases. A competitor can also reach a similar footprint using standard pruning and quantization tooling, so the real advantage here is the tuned end-to-end framework rather than the individual techniques. As a result, Ultron's invention enables high-quality detection to run locally on constrained hardware, improving responsiveness and resilience while reducing the hardware and infrastructure burden of large-scale rollout."
              }
            }
          ]
        },
          ...
          inventions: [
            {
              ...
              full: {
                ...
                superior: "A competitor recovering the same accuracy without this method typically scales the backbone up, which raises compute and latency, and heavier models push toward higher-cost edge hardware or cloud inference. As a result, Ultron's invention delivers the needed accuracy at a smaller backbone size, helping the system run efficiently on lane hardware without degrading response time or rollout economics."
              }
            },
            {
              id: "lean-cnn",
        ...
        },
        {
          ...
          inventions: [
            ...
            },
            {
              ...
              full: {
                accomplishes: "At a high level, this technology provides a zero-shot approach to enroll and stop the continuous, unnecessary false-positive detection alerts that appear in any real working system. Any deployed system will have certain outlier detections — they happen whether we expect them or not. The ability to simply zero-shot enroll a false positive stops the system from repeating that known detection/recognition mistake. Practically, it maintains a gallery of human-confirmed false-positive feature representations, which are then suppressed in real-time operation (without any retraining or operational delay once confirmed), so similar false-positive detections during inference are ignored via similarity matching. This provides an elegant, instantaneous mechanism to band-aid the unpredictable, allowing for a later model retraining sometime in the future without disrupting normal operation of the AI system.",
                works: "By explicitly modeling newly detected failure cases, the system improves precision in real time, on the fly, without any retraining — reducing any further repetition of these false alerts and avoiding more unnecessary interventions for these new edge cases.",
                superior: "A competitor could retrain the model every time false positives are discovered, but a retrain is not instantaneous even once a human operator flags it: it requires data collection, sanitation, validation, unit testing, regression analysis, re-quantization and edge optimization, and deployment — easily weeks. The other option, raising confidence thresholds, is catastrophic: increasing the threshold to suppress strong, confident false positives also suppresses legitimate detections (false negatives), which can render the system unable to detect real products passing through checkout. As a result, Ultron's invention suppresses known error patterns instantly without retraining, so the system does not keep firing the same false alerts for weeks while a retrain is pending — avoiding the operational friction, lost trust, and eventual removal that has killed many deployed systems."
              }
            }
          ]
        }
